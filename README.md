...
- **SQLite** via `better-sqlite3`, schema with **Drizzle ORM**
...
# 3. apply schema + seed sample data
pnpm db:push          # creates data/canvass.db from drizzle/0000_*.sql
pnpm seed             # 30 Boulder, CO addresses (~30s, Photon rate limit)
...
| `pnpm db:generate` | regenerate `drizzle/*.sql` from `src/lib/db/schema.ts` |
| `pnpm db:push` | apply current schema to `data/canvass.db` |
| `pnpm seed` | clear + seed Boulder addresses, people, and some visits |
...
The `canvasing:setup` devenv task chains `db:generate && db:push` for one-shot DB provisioning.
...
| `/import` | upload a CSV of residents. Auto-maps common column names (`firstName`, `first`, `fname`, `givenName`, etc.). Required: first name, last name, street, city, state, zip. Optional: age, party. Geocodes new addresses via Photon. |
...
- `addresses(id, lat, lng, street, city, state, zip, geocodedAt, createdAt)`
- `people(id, addressId → addresses, firstName, lastName, age, party, source)`
- `visits(id, addressId → addresses, personId? → people, visitedAt, homeStatus, talked, interested?, notes?, contactEmail?, contactPhone?)`
- `homeStatus` enum: `home | not_home | unknown`
- `imports(id, filename, uploadedAt, rowCount, status, error?)`
- `imports.status` enum: `pending | complete | failed`

All FKs cascade-delete from address and `visits.personId` is `ON DELETE SET NULL`.

## Photon / rate limits

Both the seed script and the CSV import geocode through [Photon](https://photon.komoot.io) (OSM-backed, no key). The seed script sleeps 1.1s between requests out of politeness; importing a large CSV takes roughly `N + 1` seconds.
...
src/
  ...
  routes/
    ...
    import.tsx             # CSV upload (form action)
    [...404].tsx
  lib/
    components/
    ...
    db/
      ...
      client.ts            # better-sqlite3 connection (WAL, FK on, auto-mkdir)
      migrate.ts           # runMigrations() helper
      seed.ts              # Boulder, CO seed
    server/
...
data/canvass.db            # local SQLite (gitignored)
...
- `pnpm install` triggers native builds for `@parcel/watcher`, `better-sqlite3`, and `esbuild`. The list lives in `pnpm-workspace.yaml` under `allowBuilds` because pnpm 11 removed the package.json `pnpm.onlyBuiltDependencies` field and global `~/.config/pnpm/config.yaml` won't accept it.
- `drizzle-kit push` is interactive — it prompts before destructive changes. In CI / non-TTY use `pnpm db:push` from a TTY, or run the migration via `tsx -e 'import("./src/lib/db/migrate").then(m => m.runMigrations())'`.
- `src/lib/db/seed.ts` runs `runMigrations()` itself before clearing data, so re-running the seed is safe.
