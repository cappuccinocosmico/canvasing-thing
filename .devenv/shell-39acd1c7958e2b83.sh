...
exec tsx -e '
const r = await fetch("https://nominatim.openstreetmap.org/search?q=1234+Pearl+St,+Boulder,+CO+80302&format=json&limit=1&email=demo@example.com", { headers: { "User-Agent": "test/1.0" } });
console.log("status:", r.status);
const j = await r.json();
console.log("result:", j.length, j[0]?.lat, j[0]?.lon);
'