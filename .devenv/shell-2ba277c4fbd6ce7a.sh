...
exec tsx -e '
const { importCsv } = await import("./src/lib/server/csv.ts");
const fs = await import("node:fs/promises");
const text = await fs.readFile("/tmp/opencode/test-import.csv", "utf8");
const result = await importCsv("test-import.csv", text);
